...
function createProxyConfig(env: Record<string, string>) {
  ...
  // SSO 服务代理（优先级更高，匹配 /api/sso）
  // 前端: /api/sso/getCaptcha -> 后端: http://192.168.80.203:19999/sso/sso/getCaptcha
  if (env.VITE_APP_SSO_API_URL) {
    proxyConfig["/api/sso"] = {
      changeOrigin: true,
      target: env.VITE_APP_SSO_API_URL,
      rewrite: (path: string) => path.replace(/^\/api/, ""),
      // /api/sso/getCaptcha -> /sso/getCaptcha
      // 目标: http://192.168.80.203:19999/sso + /sso/getCaptcha = http://192.168.80.203:19999/sso/sso/getCaptcha
    };
  }

  // SAS 服务代理（优先级更高，匹配 /api/sas）
  if (env.VITE_APP_SAS_API_URL) {
    proxyConfig["/api/sas"] = {
      changeOrigin: true,
      target: env.VITE_APP_SAS_API_URL,
      rewrite: (path: string) => path.replace(/^\/api/, ""),
    };
  }
