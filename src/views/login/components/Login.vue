<template>
  <div class="auth-panel-form">
    ...
    >
      <!-- 员工工号 -->
      <el-form-item prop="staffJobNumber">
        <el-input v-model.trim="loginFormData.staffJobNumber" :placeholder="t('login.username')">
          <template #prefix>
      ...
      <el-form-item prop="captcha">
        <div flex items-center gap-10px>
          ...
          <div cursor-pointer h-44px w-140px flex-center @click="getCaptcha">
            ...
            <el-text v-else-if="captchaError" type="danger" size="small">
              加载失败，点击重试
            </el-text>
            <img
...
const captchaError = ref(false); // 防止无限循环
...
function getCaptcha() {
  ...
  AuthAPI.getCaptcha()
    .then((data) => {
      ...
      // 确保 Base64 图片格式正确
      let imageData = data.captchaImage;
      if (imageData && !imageData.startsWith("data:image")) {
        imageData = `data:image/png;base64,${imageData}`;
      }

      captchaBase64.value = imageData;
    })
    .catch((error) => {
